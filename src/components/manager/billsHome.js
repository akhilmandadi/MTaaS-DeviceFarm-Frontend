...
class Bills extends Component {
    ...
    onChangeBillingPeriod = (e) => {
      var res = this.props.deviceMindetails.filter(deviceMindetail => deviceMindetail.projectName === this.state.selectedProject);
      if(res != []){
        res = res[0].data.filter(dataElement => dataElement.name === e.target.value);
        console.log(res);
        if (e.target.value == 'all') {
          this.setState({
            selectedBillingPeriod: 'all',
                    ...
                    <div style={{ padding:"0px 0px 70px 0px" }}>
                        ...
                        <div className="dropdown" style={{float:"right" }}>
                            <select ref="userInput"
                                ...
                                {
                                    this.props.billingPeriods.map((billingPeriod) => {
